...
scenario1 = {
    ...
    "step": 30,
    "satellites": [
        {"id": "STARLINK-1244", "tle1": "1 45196U 20012U   20094.58334491 -.00029761  00000-0 -20303-2 0  9999", "tle2": "2 45196  52.9956  50.6558 0001754  72.9283 341.9461 15.05609076  1716"},
        {"id": "STARLINK-1102", "tle1": "1 44920U 20001G   20094.58334491  .00050489  00000-0  34604-2 0  9999", "tle2": "2 44920  53.0009 350.6416 0001892  65.4641  85.7463 15.05571452 13382"},
        {"id": "STARLINK-1032", "tle1": "1 44737U 19074AA  20094.58334491 -.00005148  00000-0 -33442-3 0  9997", "tle2": "2 44737  53.0002 210.6425 0001072  82.8131 134.9394 15.05576243 22184"},
    ],
    "targets": [
        {"id": "stgo", "lat": -33.3833, "lon": -70.7833, "alt": 476},
        {"id": "saa", "lat": -15.0, "lon": -15, "alt": 0},
        {"id": "tokyo", "lat": 35.6830, "lon": 139.7670, "alt": 5},
...
fp_set_unix 1588054905 com_send_cmd STARLINK-1244 <- sat change
fp_set_unix 1588055655 take_daa data1; <- target command, data
                                v------<--- sat change, data;
fp_set_unix 1588069935 com_send_tm STARLINK-1032 data1; fp_set_unix 1588069935 com_send_cmd STARLINK-1032  <--- sat change
fp_set_unix 1588070715 com_send_tm tokyo data1 <- sat change, data


fp_set_unix 1588054905 com_send_cmd STARLINK-1244
    fp_set_unix 1588055655 take_daa data1;
    fp_set_unix 1588069935 com_send_tm STARLINK-1032 data1;
    fp_set_unix 1588069935 com_send_cmd STARLINK-1032
        fp_set_unix 1588070715 com_send_tm tokyo data1
...
['fp_set_unix 1588066875 com_send_cmd STARLINK-1244 ', 'fp_set_unix 1588075665 com_send_cmd STARLINK-1032 ', 'fp_set_unix 1588086435 take_data;', '']
...
Valid sequence
Backend Qt5Agg is interactive backend. Turning interactive mode on.
['fp_set_unix 1588054905 com_send_cmd STARLINK-1244 ', 'fp_set_unix 1588055655 take_data;', 'fp_set_unix 1588058475 com_send_tm data1 fp_set_unix 1588058475 com_send_cmd STARLINK-1032 ', 'fp_set_unix 1588058805 com_send_tm tokyo data1 ']

"""
def generate_commands(solution, task, scenario):
    satellites = [sat["id"] for sat in scenario.satellites]
    targets = [tgt["id"] for tgt in task.targets]
    targets_dict = {tgt["id"]: tgt for tgt in task.targets}
    data = None
    commands = []
    ...
    for i, row in solution.iterrows():
        ...
        if row['to'] in satellites:
            if data:
                line += "fp_set_unix {} com_send_tm {} ".format(time, data)
            line += "fp_set_unix {} com_send_cmd {} ".format(time, row['to'])

        # Case: target SAT->TGT use 'task target command'
        elif row['to'] in targets:
            line += "fp_set_unix {} {};".format(time, targets_dict[row['to']]['cmd'])
            # DATA <- data
        ...
        # Case: SAT -> GND use 'send_tm
            ...
            if data:
                line += "fp_set_unix {} com_send_tm {} {} ".format(time, row['to'], data)
                data = None
...
def main(scenario=scenario1, task=task1):
    ...
    if scenario.tracks_file is None or scenario.contacts_file is None:
